...
The whole problem can be traced back to determining if two or more omega-regular languages are disjoint, which is a difficult and resource intensive task for reasons discussed in [TODO ref backgr]. Consequently, the <system> only guarantees to handle the conflict, when it appears for other reasons too. In that case, the user is asked to remove one of the conflicting models, before the analysis of the behavior can proceed.
