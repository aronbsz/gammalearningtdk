...
\begin{definition}[Labeled Transition System]
	...
	\begin{itemize}
		\item $S={s_1, s_2, ..., s_n}$ the finite, non-empty set of states, $s_1$ being the initial state,
		\item $Act = {a, b, c, ...}$ the set of actions,
...
\begin{example}
	\label{ex:ltsexample}
	See Figure \ref{fig:ltsexample}. This example has three states, $S = \{s_1, s_2, s_4\}$ (hence $|S| = 3$). The initial state is $s_1$, also marked by the start arrow. The set of actions is $Act = \{money, coffee, tea\}$. Transitions are visualized as $s_1$ $\xrightarrow[]{\text{money}}$ $s_2$ given by the transition (in this example) $(s_1, money, s_2)$. The set of transitions ($\rightarrow$) also contains $(s_2, coffee, s_3)$ and $(s_2, tea, s_4)$.
\end{example}
