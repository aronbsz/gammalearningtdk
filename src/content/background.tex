...
\begin{definition}[Syntax of LTL Expressions \cite{majzikform}]
	Let $\pi = (s_0, a_1, s_1, a_2, ... )$ be a path over an LTS. Then the valid LTL expressions can be derived using the following production rules:
	\begin{itemize}
...
The corresponding semantics are defined as follows:
\begin{definition}[Semantics of LTL Expressions\cite{majzikform}]
	Let $\pi = (s_0, a_1, s_1, a_2, ... )$ be a path over an LTS model M. Then the formal semantics to the LTL expression P is given recursively, with regard to syntactic production rules as:
	\begin{itemize}
	...
	\end{itemize}
	Where $\vDash$ is te logical entailment operator, $M,\pi \vDash q$ denoting: in model $M$ over path $\pi$, $q$ holds.
\end{definition}
...
%(q_0) edge [bend left=60] node {a} (q_1)